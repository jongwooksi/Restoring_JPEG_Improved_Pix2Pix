...
class Pix2Pix:
    ...
    def generate(self, input_img):
        ...
        h9 = tf.nn.conv2d_transpose(h8, self.G_W9, output_shape=[self.batch_size, 8, 8, self.ch_G9], strides=[1, 2, 2, 1])  # [?,1,1,512] -> [?,2,2,512]
    ...
    def sample_generator(self, input_image, batch_size=1):
        ...
        h9 = tf.nn.conv2d_transpose(h8, self.G_W9, output_shape=[batch_size, 8, 8, self.ch_G9], strides=[1, 2, 2, 1])  # [?,1,1,512] -> [?,2,2,512]
        h9 = tf.nn.dropout(self.G_bn9(h9), keep_prob=self.keep_prob)
        h9 = tf.nn.relu(h9)
        h9 = tf.concat([h9, h7_], axis=3)
